"""Elastic Net model for regime-adaptive covariance estimation."""
...
class ElasticNetRegime:
    """
    Elastic Net model that predicts optimal shrinkage intensity λ
    using a selected feature set.
    """

    def __init__(self, feature_cols, alpha_grid=None, l1_ratio_grid=None):
        """
        Parameters
        ----------
        ...
        alpha_grid : list of float, optional
            Regularization strength values to search.
        l1_ratio_grid : list of float, optional
            L1 ratio values to search (0 = Ridge, 1 = Lasso).
        """
        self.feature_cols = feature_cols
        self.alpha_grid = alpha_grid or [0.001, 0.01, 0.1, 1.0, 10.0]
        self.l1_ratio_grid = l1_ratio_grid or [0.1, 0.3, 0.5, 0.7, 0.9]
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        """
        Fit Elastic Net with hyperparameter tuning on validation set.
        ...
        # Use validation set for tuning (not cross-validation to avoid look-ahead)
        best_score = np.inf
        best_model = None
        best_params = None
        ...
        for alpha in self.alpha_grid:
            for l1_ratio in self.l1_ratio_grid:
                ...
                # Suppress convergence warnings (they are harmless here)
...
def evaluate_elastic_net_on_covariance(window_data, lambdas_df, feature_cols, train_idx, val_idx, test_idx, features_df):
    """
    Train Elastic Net and evaluate covariance estimation performance.
    ...
    # Train Elastic Net
    model = ElasticNetRegime(feature_cols)
